...
Public Class AccountController
    ...
    Public Async Function Register(model As RegisterViewModel) As Task(Of ActionResult)
        If ModelState.IsValid Then
            ...
            Dim result = Nothing
            If user.UserName.ToLower().Contains("@meishi.me") = True Then
                result = Await UserManager.CreateAsync(user, model.Password)
                CreateAgentUser(user.UserName)
            Else
                result = Await UserManager.CreateAsync(user, model.Password)
            End If
    ...
    Public Sub CreateAdminIfNeeded()
        Dim AdminUserName As String = ConfigurationManager.AppSettings("AdminUserName")
        Dim AdminPassword As String = ConfigurationManager.AppSettings("AdminPassword")
        Dim objAdminUser = UserManager.FindByEmail(AdminUserName)
        Dim objRole = Nothing
        If objAdminUser IsNot Nothing Then
            objRole = RoleManager.FindById(objAdminUser.Id)
        End If
        If objRole Is Nothing Then

            If Not RoleManager.RoleExists("Administrator") Then
                Dim objAdminRole As IdentityRole = New IdentityRole("Administrator")
            ...
            Dim AdminUserCreateResult = UserManager.Create(objNewAdminUser, AdminPassword)
            If objAdminUser Is Nothing Then
    ...
    Private Sub CreateAgentUser(ByVal AgentUserName As String)

        Dim objAgentUser = UserManager.FindByEmail(AgentUserName)

        If objAgentUser IsNot Nothing Then

            If Not RoleManager.RoleExists("Agent") Then
                Dim objAgentRole As IdentityRole = New IdentityRole("Agent")
                RoleManager.Create(objAgentRole)
            End If

            'Dim objNewAgentUser = New ApplicationUser With {
            '    .UserName = AgentUserName,
            '    .Email = AgentUserName
            '}

            'Dim AgentUserCreateResult As IdentityResult = UserManager.Create(objNewAgentUser, AgentPassword)
            UserManager.AddToRole(objAgentUser.Id, "Agent")
